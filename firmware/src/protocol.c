...
/**
 * @file protocol.c
 * @brief Wire protocol implementation
 */
#include "protocol.h"
#include <string.h>

// CRC32 table (IEEE 802.3 polynomial 0x04C11DB7)
static const uint32_t crc32_table[256] = {
    ...
    0x316E8EEF, 0x4669BE79, 0xCB61B38C, 0xBC66831A, 0x256FD2A0, 0x5268E232,
...
ps_error_e ps_encode_frame(uint8_t type, const void *payload, uint16_t payload_len,
    ...
    if (payload_len > PS_MAX_PAYLOAD || out_buf == NULL || out_len == NULL) {
    ...
    *p++ = (payload_len >> 8) & 0xFF;
    ...
    uint32_t crc = ps_crc32(out_buf, p - out_buf);
    *p++ = crc & 0xFF;
    *p++ = (crc >> 8) & 0xFF;
    *p++ = (crc >> 16) & 0xFF;
    *p++ = (crc >> 24) & 0xFF;
...
ps_error_e ps_decode_frame(const uint8_t *buf, size_t len,
    ...
    if (len < PS_FRAME_OVERHEAD) {
        return PS_ERR_PROTOCOL;
    }

    if (buf[0] != PS_SOF || buf[len - 1] != PS_EOF) {
        return PS_ERR_PROTOCOL;
    }

    uint16_t payload_len = buf[2] | (buf[3] << 8);
    if (payload_len > PS_MAX_PAYLOAD || len != payload_len + PS_FRAME_OVERHEAD) {
        return PS_ERR_PROTOCOL;
    }

    // Verify CRC
    uint32_t crc_calc = ps_crc32(buf, len - 5);
    uint32_t crc_recv = buf[len - 5] | (buf[len - 4] << 8) |
                        (buf[len - 3] << 16) | (buf[len - 2] << 24);
...
ps_error_e ps_validate_frame(const uint8_t *buf, size_t len) {
    ...
    return ps_decode_frame(buf, len, &type, &payload, &payload_len);
}